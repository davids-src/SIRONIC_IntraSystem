...
Megjegyzés: A jótállási idő meghosszabbodik a kijavításra átadás napjától kezdve azzal az idővel, amely alatt a fogyasztó a terméket a hiba miatt rendeltetésszerűen nem használhatta.
...
Fogyasztói jogvita esetén a fogyasztó a lakóhelye vagy tartózkodási helye szerinti vármegyei kereskedelmi és iparkamarák mellett működő Békéltető Testülethez fordulhat.
Az illetékes Békéltető Testületek elérhetőségei: www.bekeltetes.hu`;

/**
 * GET /api/warranties/:id/pdf-data
 * PDF generáláshoz szükséges adatok (csak saját jótállási jegyhez)
 */
export async function GET(
  ...
  try {
    ...
    return await withDb(async () => {
      const warranty = await WarrantyCardModel.findOne({
        _id: id,
        tenantId,
        contact_id: contactId, // Csak a saját jótállása
      }).lean();

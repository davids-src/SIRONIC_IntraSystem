  ...
  try {
    ...
    return await withDb(async () => {
      const user = (await PortalUserModel.findOne({
        email,
      }).lean()) as PortalMagicUserLean | null;
      if (
        !user ||
        !(user.roleKeys ?? []).some(
          (k) => k === "partner.admin" || k === "partner.viewer",
        )
      ) {
      ...
      await sendEmail({
        to: email,
        subject: "SIRONIC Partner Portal - egyszeri bejelentkezési link",
        template: React.createElement(
          ...
          React.createElement(
            "p",
            null,
            "A lenti gombra kattintva 15 percen belul be tudsz jelentkezni a SIRONIC Partner Portal rendszerbe.",
          ),
          React.createElement(
            "p",
            null,

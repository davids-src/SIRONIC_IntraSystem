...
import { DomainHostingRecordModel, serializeForJson } from "@crm/db";
...
export async function GET(_req: Request, ctx: RouteCtx) {
  try {
    ...
    return await withDb(async () => {
      const doc = await DomainHostingRecordModel.findOne({
        _id: id,
...
export async function PATCH(req: Request, ctx: RouteCtx) {
  try {
    ...
    return await withDb(async () => {
      const doc = await DomainHostingRecordModel.findOneAndUpdate(
...
export async function DELETE(_req: Request, ctx: RouteCtx) {
  try {
    ...
    guard(actor, { module: "contact", action: "write", scope: "global" });
    return await withDb(async () => {
      const res = await DomainHostingRecordModel.deleteOne({
        _id: id,
        tenantId: actor.tenantId,
      });

...
export async function GET(_req: Request, ctx: RouteCtx) {
  try {
    ...
    return await withDb(async () => {
      const doc = await TicketModel.findOne({ _id: id, tenantId: actor.tenantId }).lean();
      if (!doc) {
...
export async function DELETE(_req: Request, ctx: RouteCtx) {
  try {
    const { id } = await ctx.params;
    const { actor } = await requireCrmAuth();
    guard(actor, { module: "ticket", action: "admin", scope: "global" });
    return await withDb(async () => {
      const res = await TicketModel.deleteOne({ _id: id, tenantId: actor.tenantId });
